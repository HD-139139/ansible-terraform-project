resource "aws_instance" "my_instance" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      echo "${self.public_ip} ansbile_ssh_user=ubuntu ansible_ssh_private_key_file=./my_sshkey" > inventory.ini
      echo "private_ip: ${self.public_ip}" >> group_vars/all.yaml
      echo "service_port: ${var.service_port}" >> group_vars/all.yaml
      sudo apt-get update
      EOF
  }
